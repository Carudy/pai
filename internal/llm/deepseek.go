...
func (p *deepSeekProvider) Completion(ctx context.Context, params CompletionParams) (*ChatCompletion, error) {
	if params.Model == "" {
	...
	return parseDeepSeekCompletion(rawBody), nil
}

func (p *deepSeekProvider) CompletionStream(ctx context.Context, params CompletionParams) (<-chan ChatCompletionChunk, <-chan error) {
	...
	bodyBytes, err := json.Marshal(bodyMap)
	if err != nil {
		errChan <- fmt.Errorf("deepseek: marshal request: %w", err)
		close(chunkChan)
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.baseURL+"/chat/completions", bytes.NewReader(bodyBytes))
	if err != nil {
		errChan <- fmt.Errorf("deepseek: create request: %w", err)
		close(chunkChan)
		return chunkChan, errChan
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		errChan <- fmt.Errorf("deepseek: do request: %w", err)
		close(chunkChan)
		return chunkChan, errChan
	}
	...
	go func() {
		...
		for scanner.Scan() {
			...
			chunk := parseDeepSeekChunk(data)
			chunkChan <- chunk
		}

		if err := scanner.Err(); err != nil {
			errChan <- fmt.Errorf("deepseek: scan stream: %w", err)
		} else {
			errChan <- nil
		}
	}()
...
// parseDeepSeekCompletion parses a blocking response JSON body into a ChatCompletion,
// extracting reasoning_content from the raw JSON using gjson.
func parseDeepSeekCompletion(raw []byte) *ChatCompletion {
	...
	choices.ForEach(func(_, choice gjson.Result) bool {
		...
		// Extract reasoning_content (non-standard DeepSeek field)
...
// parseDeepSeekChunk parses a single SSE data line into a ChatCompletionChunk,
// extracting reasoning_content from the raw JSON using gjson.
func parseDeepSeekChunk(data string) ChatCompletionChunk {
