...
// mistralProvider implements Provider for the Mistral API (OpenAI-compatible).
...
func (p *mistralProvider) Completion(ctx context.Context, params CompletionParams) (*ChatCompletion, error) {
	if params.Model == "" {
	...
	if err != nil {
		return nil, fmt.Errorf("mistral: do request: %w", err)
	}
	...
	return parseMistralCompletion(rawBody), nil
}

func (p *mistralProvider) CompletionStream(ctx context.Context, params CompletionParams) (<-chan ChatCompletionChunk, <-chan error) {
	...
	bodyBytes, err := json.Marshal(bodyMap)
	if err != nil {
		errChan <- fmt.Errorf("mistral: marshal request: %w", err)
		close(chunkChan)
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, p.baseURL+"/v1/chat/completions", bytes.NewReader(bodyBytes))
	if err != nil {
		errChan <- fmt.Errorf("mistral: create request: %w", err)
		close(chunkChan)
		return chunkChan, errChan
	}
	req.Header.Set("Content-Type", "application/json")
	...
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
		errChan <- fmt.Errorf("mistral: do request: %w", err)
		close(chunkChan)
		return chunkChan, errChan
	}
	...
	go func() {
		...
		for scanner.Scan() {
			...
			chunk := parseMistralChunk(data)
			chunkChan <- chunk
		}

		if err := scanner.Err(); err != nil {
			errChan <- fmt.Errorf("mistral: scan stream: %w", err)
		} else {
			errChan <- nil
		}
	}()
...
// parseMistralCompletion parses a standard OpenAI-compatible blocking response.
func parseMistralCompletion(raw []byte) *ChatCompletion {
...
// parseMistralChunk parses a standard OpenAI-compatible streaming chunk.
func parseMistralChunk(data string) ChatCompletionChunk {
